...
public class EventoDetalles extends AppCompatActivity {
    ...
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
        super.onRestoreInstanceState(savedInstanceState); // Si hay una subida en progreso, obtiene su referencia y crea una nueva StorageReference
        final String stringRef = savedInstanceState.getString("reference");
        if (stringRef == null) {
            return;
        }
        mStorageRef = FirebaseStorage.getInstance().getReferenceFromUrl(stringRef); // Recupera todas las UploadTasks para la StorageReference guardada.
        List<UploadTask> tasks = mStorageRef.getActiveUploadTasks();
        if (tasks.size() > 0) { // Obtiene la tarea que minitoriza la subida.
            UploadTask task = tasks.get(0); // Añade nuevos escuchadores a la tarea usando un ámbito de la actividad.
